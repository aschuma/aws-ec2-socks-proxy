...
def awssocks_ami_image():
    try:
        ...
        if len(amzn2_amis) > 0:
            ...
            logging.info(str(amzn2_amis[0]))
            return ami_image_id
        elif len(ami_params) > 0:
            ami_image_id = ami_params['Parameters'][0]['Value']
            logger.info("Found an Amazon Machine Image (AMI) to use for the proxy.")
            logging.info(str(ami_params[0]))
            return ami_image_id
        else:
            raise RuntimeError(
        ...
        logging.exception("AMI - Lookup failed")
        raise
